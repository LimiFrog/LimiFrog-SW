...
extern bool 	ConversionComplete;
...
*                  Used by ADC2 in this project
*******************************************************************************/
void DMA1_Channel2_IRQHandler(void)
{

    HAL_DMA_IRQHandler( &hDma1 );
  // Checks if hDma1 is source of IT
  // if so: HAL clears the IT and calls back functions pointed by
  // hdma->XferCpltCallback / XferHalfCpltCallback / XferErrorCallback;
  // These were set to ADC_DMAConvCplt / ADC_DMAHalfConvCplt / ADC_DMAError
  // by function HAL_ADC_Start_DMA
  // which in turn call HAL_ADC_ConvCpltCallback(hadc), 
  // HAL_ADC_ConvHalfCpltCallback(hadc) or HAL_ADC_ErrorCallback(hadc)
}
