...
void 	LBF_Mic_Setup_ADC2_TIM6(void);
...
/* ==  Body     ======================================== */
	...
	if ( __HAL_ADC_GET_FLAG( &hAdc2, ADC_FLAG_AWD1 ) )  // Flag AWD1 was set by Analog Watchdog 
	{
	    LBF_Led_ON();
 	    ...
 	    if (Bkgnd_is_cyan)
	    {
		LBF_OLED_Fill(0, 0, 160, 128, BLACK);
		Bkgnd_is_cyan = FALSE;
	    }
	    else
	    {
...
ADC_AnalogWDGConfTypeDef AnalogWDGConfig;
   ...
   //Timer6 Init 
    LBF_Timer_Setup( TIMER6, TIMER_UNIT_US, 50 );  // Set Timer Period=50 us i.e. fqcy = 20KHz
