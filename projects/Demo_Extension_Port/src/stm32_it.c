...
extern bool volatile TxDMA_complete_UART4 , RxDMA_complete_UART4 ;
...
// On LimiFrog-0.2, possible external interrupts expected here would be :
//  - INT1_ACC_GYR_PIN, Interrupt from LSM6DS3 (PC6)
//  - CONN_POS1_PIN (PA6), CONN_POS10_PIN (PB8), CONN_POS11_PIN (PB9)
...
void EXTI9_5_IRQHandler(void)
{
    ...
    if ( LBF_IsPending_EXTI_ExtPort( CONN_POS10_PIN ) )
    {
	IRQ_Seen_Position10 = true;
	LBF_Clear_EXTI_ExtPort( CONN_POS10_PIN );
    }
}

// NOT using HAL-style Interrupt Service Routine (ISR) here,
// contrary to what's done for e.g. ONOFF_STAT_PIN below
...
// This is the "HAL way" to do it. Fairly simple and systematic but
// some redundancies. Alternatively could do the processing required in IRQ 
...
// UART4 Rx is the source of this interrupt
  ...
  if(LL_DMA_IsActiveFlag_TC5(DMA2))  // Transfer Complete IT
  {
    LL_DMA_ClearFlag_GI5(DMA2);  // global clear of all channel5 DMA2 IT flags
    RxDMA_complete_UART4 = true;
  }
  else if(LL_DMA_IsActiveFlag_TE5(DMA2))  // Error
  {
    /* Call Error function */
  }
